---
title: "automated-redcap-pull-data"
description: |
...
This will open up a file named `.Renviron` in your R environment. Add the following line to the file:
...
In the next code chunk, you can use the `redcap_read` function from REDCapTidieR to pull data from your REDCap project. You will need to know the URL of your REDCap project, the API token, and the name of the project. You can use the following code to pull the data:
...
The next part is up to you. Summarize the data in any way you like. You can use the `janitor` package to clean up the data, the `gt` package to make nice tables, and the `tidyverse` to summarize the data.
...
[Link to HTML Dashboard](https://phiggins.quarto.pub/smart-dashboard-e99e/)
...
2.  `/app/apps/rhel8/R/4.3.2/bin/` locates the R file, then executes an R script enclosed in quotes. This one is just `rmarkdown::render`, which renders the qmd document enclosed in single quotes. 
...
You probably want to test this out first, to make sure it works. Use a time about 2 minutes ahead of the current time. 
