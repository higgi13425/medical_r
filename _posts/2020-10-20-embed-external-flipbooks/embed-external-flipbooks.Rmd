...
Just insert some *iframe* code between two comment fences, as in the code chunk below. This is actually *easier* than inserting a *learnr* app - with no resizing messes.
...
Note that if you have multiple iframes in your HTML, each one **must have** a unique id.
...
NOTE: Your *iframe* tag, with many options, should only have spaces between options, with **no** return characters. This will make your iframe silently fail.
...
And your micro-flipbook should look something like this: *Note that it will look like an empty box in the RStudio viewer, but trust:*
...
in a minute or two, you will have a working flipbook on the web that looks something like the below example.
